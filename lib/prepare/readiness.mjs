...
function normalizeLevel(value) {
  return ["honors", "ap", "ib", "college"].includes(value) ? value : "regular";
}
...
export function inferCourseFromName(name) {
  ...
  if (/\b(ap|advanced placement)\b/.test(text)) inference.level = "ap";
  else if (/\bib\b/.test(text)) inference.level = "ib";
  else if (/\b(dual|college|community college|transferable)\b/.test(text)) {
    inference.level = "college";
  } else if (/\b(honors|honour)\b/.test(text)) {
    inference.level = "honors";
  }

  if (/\b(english|literature|composition|writing)\b/.test(text)) {
    inference.agCategory = "b";
  } else if (
    /\b(algebra|geometry|precalculus|pre-calculus|calculus|statistics|stats|math)\b/.test(
      text,
    )
  ) {
    inference.agCategory = "c";
  } else if (
    /\b(biology|chemistry|physics|anatomy|physiology|environmental science|science)\b/.test(
      text,
    )
  ) {
    inference.agCategory = "d";
  } else if (
    /\b(history|government|civics|world history|us history|u\.s\. history)\b/.test(
      text,
    )
  ) {
    inference.agCategory = "a";
  } else if (/\b(spanish|french|mandarin|chinese|latin|language)\b/.test(text)) {
    inference.agCategory = "e";
  } else if (/\b(art|music|theater|theatre|dance|visual|performing)\b/.test(text)) {
    inference.agCategory = "f";
  } else if (/\b(psychology|economics|sociology|computer science)\b/.test(text)) {
    inference.agCategory = "g";
  }

  return inference;
}

export function buildCoursesFromCourseEntries(entries) {
  if (!Array.isArray(entries)) return [];

  return entries
    .filter((entry) => entry && typeof entry.name === "string")
    .map((entry) => {
      const inferred = inferCourseFromName(entry.name);
      const agCategory =
        entry.agCategory === "unknown"
          ? null
          : normalizeCategory(entry.agCategory);
      const intendedAgCategory =
        agCategory ?? normalizeCategory(inferred.agCategory);

      return {
        id: entry.id,
        name: entry.name,
        agCategory:
          entry.verificationStatus === "confirmed" ? agCategory : null,
        intendedAgCategory,
        status: entry.status,
        units: 1,
        grade: entry.grade,
        ...
        honorsType:
          normalizeLevel(entry.level) === "college"
...
export function summarizeCourseInventory(entries) {
  const courseEntries = Array.isArray(entries) ? entries : [];
  const advancedLevels = new Set(["honors", "ap", "ib", "college"]);
  const completed = courseEntries.filter((entry) => entry.status === "completed");
  const currentOrPlanned = courseEntries.filter(
    (entry) => entry.status === "current" || entry.status === "planned",
  );
  const advancedCourses = courseEntries.filter((entry) =>
    advancedLevels.has(entry.level),
  );
  const outsideCourses = courseEntries.filter(
    (entry) => entry.source !== "high_school",
  );
  ...
  const mathScienceCourses = courseEntries.filter((entry) => {
    const inferred = inferCourseFromName(entry.name);
    const category =
      entry.agCategory === "unknown" ? inferred.agCategory : entry.agCategory;
  ...
  return {
    ...
    outsideCourseCount: outsideCourses.length,
    needsVerificationCount: needsVerification.length,
    mathScienceCount: mathScienceCourses.length,
    notes: [
      courseEntries.length
        ? "Course entries create a stronger report than rough category counts."
        : "Add course rows to replace the rough A-G counters with a course-based review.",
      needsVerification.length
        ? "Some courses need counselor or official A-G list confirmation before treating them as UC-counted."
        : "Entered courses marked confirmed can be used for a cleaner first-pass review.",
      outsideCourses.length
        ? "Out-of-school courses are separated so they can be treated as A-G, college, or enrichment only after verification."
        : "Out-of-school learning can be added without assuming it counts for UC.",
    ],
  };
}

export function buildSeniorCourseConsiderations(entries, baseline = {}) {
  const summary = summarizeCourseInventory(entries);
  const courses = Array.isArray(entries) ? entries : [];
  const hasAdvancedScience = courses.some((entry) => {
    const name = String(entry.name ?? "").toLowerCase();
    return (
      (entry.level === "ap" || entry.level === "ib" || entry.level === "college") &&
      /\b(biology|chemistry|physics|environmental|anatomy|physiology)\b/.test(name)
    );
  });
  const hasStats = courses.some((entry) =>
    /\b(statistics|stats)\b/i.test(entry.name ?? ""),
  );
  const hasCalculus = courses.some((entry) =>
    /\b(calculus)\b/i.test(entry.name ?? ""),
  );
  ...
  if (baseline.seniorMathPlan === "not-sure") {
    considerations.push({
      id: "choose-senior-math",
      title: "Choose a senior-year math option to discuss",
      detail:
        "Calculus and statistics can both be reasonable depending on school sequence, interests, and workload.",
      verify: true,
    });
  } else if (baseline.seniorMathPlan === "statistics" && !hasStats) {
    considerations.push({
      id: "review-statistics",
      title: "Review whether statistics supports her likely paths",
      detail:
        "Statistics can support health, biology, public health, data, psychology, and social-science interests.",
      verify: true,
    });
  } else if (baseline.seniorMathPlan === "calculus" && !hasCalculus) {
    considerations.push({
      id: "review-calculus",
      title: "Review whether calculus is sustainable and useful",
      detail:
        "Calculus is useful for many quantitative paths, but the right choice should fit mastery and workload.",
      verify: true,
    });
  }

  if (!hasAdvancedScience && summary.mathScienceCount >= 3) {
    considerations.push({
      id: "advanced-science",
      title: "Consider one advanced science if available and sustainable",
      detail:
        "For medicine, biology, and health interests, advanced biology, chemistry, physics, or anatomy can be useful preparation.",
      verify: true,
    });
  }
  ...
  if (summary.needsVerificationCount > 0) {
    considerations.push({
      id: "verify-before-choosing",
      title: "Verify uncertain courses before locking senior year",
      detail:
        "Course status, A-G category, and honors-point assumptions should be checked before using them to shape the plan.",
      verify: true,
    });
  }
  ...
  considerations.push({
    id: "keep-balance",
    title: "Keep the senior schedule strong but humane",
    detail:
      "The goal is a thoughtful course pattern, not the maximum possible number of advanced courses.",
    verify: false,
  });
...
export function buildReadinessSnapshot({
  ...
  const hasCourseEntries = Array.isArray(baseline.courseEntries)
    && baseline.courseEntries.length > 0;
  const gpaCourses = hasCourseEntries
  ...
  return {
    ...
    agAudit,
    gpaEstimate,
    courseInventory: summarizeCourseInventory(baseline.courseEntries),
    seniorCourseConsiderations: buildSeniorCourseConsiderations(
      baseline.courseEntries,
      baseline,
    ),
    findings,
    nextActions,
